...
As an example, we use the street network of the city center of Münster, Germany. It is loaded as an object of class `sf`, and contains only `LINESTRING` geometries, without any additional variables. The same steps, however, can be used for `sf` objects that contain any number of additional variables.
...
### Step 1: Give each edge an unique index
...
### Step 2: Create nodes at the start and end point of each edge
...
Now each of the start and endpoints from step has been assigned a node ID in step 3, we can add the node indices to the edges. In other words, we can specify for each edge, in which node it starts, and in which node it ends.
...
For make the approach more convenient, we can combine all steps above into a single function, that takes an `sf` object with `LINESTRING` geometries as input, and returns a spatial `tbl_graph`.

```{r}
sf_to_tidygraph = function(x) {
...
sf_to_tidygraph(muenster_center)
```

## Combining the best of both worlds
Having the data stored in the tbl_graph structure, with a geometry list column for both the edges and nodes, enables us to combine the wide range of functionalities in sf and tidygraph, in a way that fits neatly into the tidyverse.
...
With one flow of pipes, we can 'escape' the graph structure, and turn either the edges and nodes back into real `sf` objects, and, for example, summarise the data based on a specific variable.
...
ggplot() +
  ...
  geom_sf(
    data = graph %>% activate(nodes) %>% as_tibble() %>% st_as_sf(),
    size = 0.5
  )
```